...
func _register_all() -> void:
	_add(_create("enemy_champion", "Enemy Champion", "champion_kill",
		Vector2i(4, 15), 60.0, "A tougher warrior approaches. Kill before it reaches the castle.", 150))
	_add(_create("archer_ambush", "Archer Ambush", "battle_training",
		Vector2i(6, 16), 45.0, "Archers appear at unexpected positions. Eliminate them all.", 120))
	_add(_create("siege_assault", "Siege Assault", "reinforced_gates",
		Vector2i(8, 18), 90.0, "Multiple siege weapons activate. Destroy them all.", 200))
	_add(_create("war_horn", "War Horn", "faster_muster",
		Vector2i(5, 15), 0.0, "A horn carrier appears briefly. One shot opportunity.", 100))
	_add(_create("siege_tower", "Siege Tower", "archer_tower",
		Vector2i(10, 20), 60.0, "A siege tower approaches. Destroy it before arrival.", 180))
	_add(_create("war_chief", "War Chief", "elite_guard",
		Vector2i(12, 20), 45.0, "An enemy commander buffs nearby warriors. Kill to break the buff.", 200))


func _create(id: String, opp_name: String, upgrade_id: String, phase_range: Vector2i, duration: float, desc: String, xp: int) -> OpportunityData:
