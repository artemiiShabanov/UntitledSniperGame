...
func _build_label(p: PaletteResource) -> void:
	theme.set_color("font_color", "Label", p.good_muted)
	theme.set_color("font_shadow_color", "Label", Color(PaletteManager.GS_DARK, 0.5))
...
func _build_button(p: PaletteResource) -> void:
	...
	var normal := _flat_box(Color(PaletteManager.GS_DARK).lightened(0.12), BUTTON_CORNER_RADIUS, Color(p.good_muted, 0.25), BUTTON_BORDER_WIDTH)
	## Hover: full accent fill — bold, unmistakable
	var hover := _flat_box(p.good_muted, BUTTON_CORNER_RADIUS, Color(p.good_muted, 0.6), BUTTON_BORDER_WIDTH)
	## Pressed: accent tint — dimmed, feels like it sinks in
	var pressed := _flat_box(Color(p.good_muted, 0.2), BUTTON_CORNER_RADIUS, Color(p.good_muted, 0.4), BUTTON_BORDER_WIDTH)
	## Disabled: muted
	var disabled := _flat_box(Color(PaletteManager.GS_DARK, 0.5), BUTTON_CORNER_RADIUS, Color(PaletteManager.GS_MID_LIGHT, 0.1), BUTTON_BORDER_WIDTH)
	## Focus: loot-tinted fill
	var focus := _flat_box(Color(p.accent_muted, 0.08), BUTTON_CORNER_RADIUS, Color(p.accent_muted, 0.4), BUTTON_BORDER_WIDTH)
	...
	theme.set_color("font_color", "Button", p.good_muted)
	theme.set_color("font_hover_color", "Button", PaletteManager.GS_DARK)
	theme.set_color("font_pressed_color", "Button", PaletteManager.GS_LIGHT)
	theme.set_color("font_disabled_color", "Button", Color(PaletteManager.GS_MID_LIGHT, 0.4))
	theme.set_color("font_focus_color", "Button", p.accent_muted)


func _build_panel(p: PaletteResource) -> void:
	## Panels: lifted fill + visible accent border for structure
	var panel_box := StyleBoxFlat.new()
	panel_box.bg_color = Color(PaletteManager.GS_DARK).lightened(0.06)
	panel_box.bg_color.a = 0.97
	panel_box.set_corner_radius_all(PANEL_CORNER_RADIUS)
	panel_box.border_color = Color(p.good_muted, 0.3)
...
func _build_progress_bar(p: PaletteResource) -> void:
	var bg := _flat_box(Color(PaletteManager.GS_DARK, 0.7), 1)
	bg.set_content_margin_all(0)
	var fill := _flat_box(p.good_muted, 1)
...
func _build_check_button(p: PaletteResource) -> void:
	theme.set_color("font_color", "CheckButton", p.good_muted)
	theme.set_color("font_hover_color", "CheckButton", PaletteManager.GS_LIGHT)
	theme.set_color("font_pressed_color", "CheckButton", p.accent_muted)
...
func _build_slider(p: PaletteResource) -> void:
	var slider_bg := _flat_box(Color(PaletteManager.GS_DARK, 0.7), 1)
	slider_bg.set_content_margin_all(4)
	var grabber_area := _flat_box(p.good_muted, 1)
	grabber_area.set_content_margin_all(4)
	var grabber_hl := _flat_box(PaletteManager.GS_LIGHT, 1)
...
func _build_line_edit(p: PaletteResource) -> void:
	var normal := _flat_box(Color(PaletteManager.GS_DARK, 0.9), 1, Color(p.good_muted, 0.15), BUTTON_BORDER_WIDTH)
	var focus := _flat_box(Color(p.good_muted, 0.08), 1, Color(p.good_muted, 0.3), BUTTON_BORDER_WIDTH)
	theme.set_stylebox("normal", "LineEdit", normal)
	theme.set_stylebox("focus", "LineEdit", focus)
	theme.set_color("font_color", "LineEdit", p.good_muted)
	theme.set_color("caret_color", "LineEdit", p.good_muted)


func _build_separator(p: PaletteResource) -> void:
	var sep_style := StyleBoxFlat.new()
	sep_style.bg_color = Color(p.good_muted, 0.25)
...
func _build_scroll_container(p: PaletteResource) -> void:
	var grabber := _flat_box(Color(p.good_muted, 0.3), 1)
	grabber.set_content_margin_all(4)
	var grabber_hl := _flat_box(Color(p.good_muted, 0.5), 1)
	grabber_hl.set_content_margin_all(4)
	var grabber_pressed := _flat_box(Color(p.good_muted, 0.7), 1)
	grabber_pressed.set_content_margin_all(4)
	var scroll_bg := _flat_box(Color(PaletteManager.GS_DARK, 0.2), 1)
